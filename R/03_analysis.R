...
load("data/TO_property.Rdata")
load("data/TO_LTM_property.Rdata")
load("data/TO_FREH.Rdata")
load("data/TO_GH.Rdata")
load("data/TO_daily.Rdata")
load("data/TO_streets.Rdata")
load("data/TO.Rdata")
...
property_in_TO <-
  property %>% 
  select(-revenue) %>% 
  st_intersection(st_buffer(TO, 250))
...
property_2016 <- 
  ...
  left_join(filter(property_in_TO, housing == TRUE), .) %>% 
  mutate(Year = "2016") %>% 
  filter(revenue > 0)
...
map <- 
  ...
  ggplot() +
  geom_sf(data = TO_streets, colour = alpha("grey", 0.5)) +
