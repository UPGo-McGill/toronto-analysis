...
TO <-
  get_census(dataset = "CA16", regions = list(CSD = "3520005"), level = "CSD",
             geo_format = "sf") %>% 
  st_transform(32617)
...
property_all <- tbl(con, "property_test")
...
daily <- 
  daily_all %>% 
  filter(property_ID %in% !! property_db$property_ID) %>% 
  collect()
...
daily <- 
  strr_expand_daily(daily, cores = 5)
...
ML_daily <- 
  strr_expand_daily(ML_daily, cores = 3)
